...
router.post("/userDocsModif", upload.fields([]), (req, res) => {
    res.set("Content-type", "application/json");
    let [status, message] = services.checkAuthZHeader(req.headers.authorization, "Bearer");
    if (status != 200) {
      res.status(status).send(message);
    ...
    if (decoded) {
      services.putDocsUserById(req.body).then(data => {
        // console.log(data);
        res.status(data.status).send(data.status);   
      })
    } else {
  ...
  // Auth : oui
// modifier profil user
router.delete("/userDocsDelete/:documentID", (req, res) => {
  ...
  if (decoded) {
    services.deletePDFById(req.params.documentID).then(data => {
      res.status(data.status).send(data.status);   
    })
  } else {
    res.status(401).end("Vous n'êtes pas authentifié");
